...
# CMSSpark modules
from CMSSpark.spark_utils import dbs_tables, phedex_tables_multiday, jm_tables_multiday, print_rows, unionAll
...
class OptionParser():
    def __init__(self):
        ...
        self.parser.add_argument("--maxtime", action="store",
            dest="maxtime", default=30, help='Select maximum time window for parsing phedex and jobMontioring in days')
...
def run(fromdate, todate, maxtime, fout, yarn=None, verbose=None, inst='GLOBAL'):
    """
    ...
    #dbs_d_df.persist(StorageLevel.MEMORY_AND_DISK)
    #dbs_f_df.persist(StorageLevel.MEMORY_AND_DISK)
    ...
    for date_pair in date_list:
        ...
        #ph_df.persist(StorageLevel.MEMORY_AND_DISK)
        ...
        ph_cols   = ['now_day', 'node_name', 'dataset_name', 'dataset_is_open', 'block_bytes', 'replica_time_create']
        ph_df_agg = ph_df.select(ph_cols)\
        ...
        #ph_df_agg.persist(StorageLevel.MEMORY_AND_DISK)
        ...
        jm_df       = tables['jm_df']
        #jm_df.persist(StorageLevel.MEMORY_AND_DISK)

        # add sortable column to job monitoring records, note jobMonitoring timeStamp in miliseconds, convert to seconds
        jm_df_date = jm_df.withColumn("job_day",        F.date_format( (F.col("StartedRunningTimeStamp")*0.001).cast('timestamp'),"yyyyMMdd"))\
                          .withColumn("job_begin_sec", (F.col("StartedRunningTimeStamp")*0.001) )\
                          .withColumn("job_end_sec",   (F.col("FinishedTimeStamp")*0.001) )
        #jm_df_date.persist(StorageLevel.MEMORY_AND_DISK)
        jm_df_date.registerTempTable('jm_df_date')
        print_rows(jm_df_date, "jm_df_date", verbose)
        jm_records  = jm_df_date.take(1)
        if verbose:
        ...
        cols = ['ddf.d_dataset AS dataset',
                ...
                'ph_df_agg.pbr_size AS pbr_size',
                'ph_df_agg.now_day AS phedex_day',
                'jm_df_date.FileName AS job_fileName',
                'jm_df_date.JobId AS job_id',
                'jm_df_date.JobExecExitCode AS job_exitCode', 
                'jm_df_date.job_begin_sec AS job_begin',
                'jm_df_date.job_end_sec AS job_end'
                ]
        ...
        stmt += 'JOIN ph_df_agg ON (ph_df_agg.node_name=jm_df_date.SiteName AND ph_df_agg.now_day=jm_df_date.job_day AND ddf.d_dataset=ph_df_agg.dataset_name) ' # Join phedex dataframe to jobMonitoring based on node_name, dateset, and phedex date
        stmt += 'WHERE jm_df_date.Type = "analysis" ' # Select on analysis type jobs
        join_df_temp = sqlContext.sql(stmt)
        #join_df_temp.persist(StorageLevel.MEMORY_AND_DISK)
        print_rows(join_df_temp, stmt, verbose)
    
        # append df table
        dfs.append(join_df_temp)
        

    # Form union of all dataframs
    print("### Forming union of all phedex jobMonitoring dataframes")
    join_df = unionAll( dfs )
    join_df.persist(StorageLevel.MEMORY_AND_DISK)

    # Use Window function to calculate Delta T between accesses on dataset
    print("### Analyzing merged dataframe")
    w_deltaT = Window.partitionBy("site", "dataset").orderBy("job_begin")

    # Delta t calculation
    f_deltaT  = F.col("job_begin").cast("long") - F.lag("job_begin", 1).over(w_deltaT).cast("long")
    ...
    f_check_dt_1day = F.when( f_deltaT>dt_1day, F.col("pbr_size") ).otherwise(0.0)
    
    # Calc deletion date for different time frames
    #f_del_date_6mth = F.when( f_deltaT>dt_6mth, F.lag("job_begin", 1).over(w_deltaT)+dt_6mth).otherwise(-1)
    #f_del_date_5mth = F.when( f_deltaT>dt_5mth, F.lag("job_begin", 1).over(w_deltaT)+dt_5mth).otherwise(-1)
    #f_del_date_4mth = F.when( f_deltaT>dt_4mth, F.lag("job_begin", 1).over(w_deltaT)+dt_4mth).otherwise(-1)
    #f_del_date_3mth = F.when( f_deltaT>dt_3mth, F.lag("job_begin", 1).over(w_deltaT)+dt_3mth).otherwise(-1)
    #f_del_date_2mth = F.when( f_deltaT>dt_2mth, F.lag("job_begin", 1).over(w_deltaT)+dt_2mth).otherwise(-1)
    #f_del_date_1mth = F.when( f_deltaT>dt_1mth, F.lag("job_begin", 1).over(w_deltaT)+dt_1mth).otherwise(-1)
    #f_del_date_1day = F.when( f_deltaT>dt_1day, F.lag("job_begin", 1).over(w_deltaT)+dt_1day).otherwise(-1)

    # Check if last access is > time window
    to_unix = jm_date_unix(jm_todate)
        
    f_check_last_6mth = F.when( ((F.col("job_begin")==F.last("job_begin").over(w_deltaT))&(to_unix-F.col("job_begin")>dt_6mth)), F.last("pbr_size").over(w_deltaT) ).otherwise(0.0)
    f_check_last_5mth = F.when( ((F.col("job_begin")==F.last("job_begin").over(w_deltaT))&(to_unix-F.col("job_begin")>dt_5mth)), F.last("pbr_size").over(w_deltaT) ).otherwise(0.0)
    f_check_last_4mth = F.when( ((F.col("job_begin")==F.last("job_begin").over(w_deltaT))&(to_unix-F.col("job_begin")>dt_4mth)), F.last("pbr_size").over(w_deltaT) ).otherwise(0.0)
    f_check_last_3mth = F.when( ((F.col("job_begin")==F.last("job_begin").over(w_deltaT))&(to_unix-F.col("job_begin")>dt_3mth)), F.last("pbr_size").over(w_deltaT) ).otherwise(0.0)
    f_check_last_2mth = F.when( ((F.col("job_begin")==F.last("job_begin").over(w_deltaT))&(to_unix-F.col("job_begin")>dt_2mth)), F.last("pbr_size").over(w_deltaT) ).otherwise(0.0)
    f_check_last_1mth = F.when( ((F.col("job_begin")==F.last("job_begin").over(w_deltaT))&(to_unix-F.col("job_begin")>dt_1mth)), F.last("pbr_size").over(w_deltaT) ).otherwise(0.0)
    f_check_last_1day = F.when( ((F.col("job_begin")==F.last("job_begin").over(w_deltaT))&(to_unix-F.col("job_begin")>dt_1day)), F.last("pbr_size").over(w_deltaT) ).otherwise(0.0)
    ...
    deltaT_df = join_df.withColumn("delta_t", f_deltaT)\
                       ...
                       .withColumn("check_dt_1day", f_check_dt_1day)\
                       ...
                       .withColumn("check_last_1day", f_check_last_1day)\
                       .fillna(0.0,"delta_t")\
                       .fillna(0.0,["check_dt_6mth","check_dt_5mth","check_dt_4mth","check_dt_3mth","check_dt_2mth","check_dt_1mth","check_dt_1day"])\
                       .fillna(0.0,["check_last_6mth","check_last_5mth","check_last_4mth","check_last_3mth","check_last_2mth","check_last_1mth","check_last_1day"])

    print_rows(deltaT_df, "deltaT_df", verbose)
    deltaT_df.persist(StorageLevel.MEMORY_AND_DISK)
    #join_df.unpersist()
            
    # perform aggregation
    print("### Aggregating final datframe")
    join_agg = deltaT_df.groupBy(['site','dataset'])\
                        .agg( F.last('pbr_size').alias('size'),
                              ...
                              F.sum('check_dt_1day').alias('total_recalled_1day'),
                              ...
                              F.sum(F.col('check_last_1day')+F.col('check_dt_1day')).alias('total_deleted_1day')  )
    
    print_rows(join_agg, "join_agg", verbose)
    join_agg.persist(StorageLevel.MEMORY_AND_DISK)
    #deltaT_df.unpersist()
    ...
    if  fout:
        ...
        join_agg.write.format("com.databricks.spark.csv")\
